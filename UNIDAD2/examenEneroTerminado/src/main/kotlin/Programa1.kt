...
class AgendaTelefonica(private val maxContactos: Int = 10) {
    ...
    fun menu() {
        ...
        println("7. Comprobar agenda llena")
        println("8. Salir")
        println("*********************")
    }
}
...
fun main() {
    println("¿Que tamaño tendrá la agenda? ")
    val agenda = AgendaTelefonica(readln().toInt())
    var salir = false
    var nombre = ""
    var telefono = ""
    ...
    while (!salir) {
        ...
        when (opc) {
            ...
            4 -> {
                println("Introduzca nombre: ")
                val msg = if(agenda.existeContacto(readln())) "Existe contacto" else "No existe contacto"
                println(msg)
            }
            5 -> {
                println("Introduzca nombre: ")
            ...
            6 -> {
                val huecos = agenda.existenHuecos()
                println("Hay $huecos contacto/s libre/s")
            }
            7 -> {
                val msg = if (agenda.estaLlena()) "La agenda está llena" else "Aún se pueden meter contactos"
                println(msg)
            }
            8 -> salir = true
